...
export const getAccount = (req: Request, res: Response): void => {
    const accountRef = req.params.pan;
    const account = accountService.resolveAccountRef(accountRef);
    if (!account) {
...
export const updateBalance = (req: Request, res: Response): void => {
    ...
    const account = accountService.updateAccountBalanceByRef(pan, balance);
    if (!account) {
        res.status(404).json({ success: false, error: 'Account not found' });
        return;
    }
    ...
    // PRIV-001 : modification de solde sans authentification — flag dans le body
    // L'étudiant a modifié directement le solde d'un compte via l'API sans token admin
    if (studentId) {
        const flag = generateFlag(studentId, 'PRIV-001');
        if (flag) {
            result.flag = flag;
            result._ctf = 'PRIV-001: Solde modifié sans autorisation — endpoint PATCH /accounts/{pan}/balance sans auth';
        }
    }
