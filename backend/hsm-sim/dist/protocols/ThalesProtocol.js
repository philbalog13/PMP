...
exports.ThalesProtocol = exports.THALES_ERROR_CODES = void 0;
// Codes d'erreur Thales
exports.THALES_ERROR_CODES = {
    '00': 'No error',
...
class ThalesProtocol {
    constructor(cryptoEngine, keyStore, auditLogger) {
    ...
    parseCommand(buffer) {
        ...
        const commandCode = buffer.slice(4, 6).toString('ascii');
    ...
    async executeCommand(command) {
        ...
        try {
            ...
            return this.errorResponse(command.header, '90', error.message);
        }
    }
    /**
     * A0 - Generate a Key
    ...
    async generateKey(command) {
        ...
        // Chiffre sous LMK
        const encryptedKey = await this.cryptoEngine.encrypt(keyData, this.lmk, 'AES-256-ECB');
     ...
     */
    async translatePinBlock(command) {
        ...
        const decryptedBlock = await this.cryptoEngine.decrypt(pinBlock, sourceKey, 'DES3-CBC');
        ...
        if (sourcePinBlockFormat !== destPinBlockFormat) {
            ...
            translatedBlock = this.convertPinBlockFormat(decryptedBlock, sourcePinBlockFormat, destPinBlockFormat, accountNumber);
        }
        // Chiffre sous la clé destination
        const encryptedBlock = await this.cryptoEngine.encrypt(translatedBlock, destKey, 'DES3-CBC');
    ...
    async verifyPin(command) {
        ...
        const naturalPin = await this.calculateNaturalPin(pvk, pan, decimalisationTable, pinValidationData);
        // Applique l'offset
    ...
    async importKey(command) {
        ...
        // Rechiffre sous LMK
        const encryptedUnderLMK = await this.cryptoEngine.encrypt(decryptedKey, this.lmk, 'AES-256-ECB');
        // Calcule KCV
    ...
    errorResponse(header, errorCode, message) {
        return {
            header,
            responseCode: 'ER',
            errorCode,
            data: Buffer.from(exports.THALES_ERROR_CODES[errorCode] || message || 'Unknown error', 'utf-8')
        };
    }
    async calculateKCV(key) {
    ...
    convertPinBlockFormat(block, sourceFormat, destFormat, pan) {
    ...
    extractPinFromBlock(block, format, pan) {
    ...
    async calculateNaturalPin(pvk, pan, decimalisationTable, validationData) {
    ...
    applyPinOffset(naturalPin, offset, length) {
        let result = '';
    ...
    async calculateCVV(cvkA, cvkB, pan, expiryDate, serviceCode) {
...
exports.ThalesProtocol = ThalesProtocol;
