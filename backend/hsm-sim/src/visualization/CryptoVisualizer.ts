...
// Session de visualisation complète
...
export class CryptoVisualizer extends EventEmitter {
     ...
     */
    startSession(type: VisualizationType, title: string): string {
     ...
     */
    addStep(step: Omit<VisualizationStep, 'stepNumber'>): void {
        if (!this.currentSession) {
            throw new Error('No active visualization session');
        }

        const fullStep: VisualizationStep = {
            ...step,
     ...
     */
    endSession(status: 'COMPLETED' | 'ERROR' = 'COMPLETED'): VisualizationSession | null {
     ...
     */
    visualizePinBlockFormat0(pin: string, pan: string): VisualizationSession {
     ...
     */
    visualizeRetailMac(message: string, key: string): VisualizationSession {
     ...
     */
    visualizeCvvGeneration(pan: string, expiry: string, serviceCode: string): VisualizationSession {
     ...
     */
    visualizeDukptDerivation(bdk: string, ksn: string): VisualizationSession {
    ...
    private formatHexDump(hex: string): string {
        let dump = '';
    ...
    private formatBitXor(a: string, b: string, result: string): string {
    ...
    getSession(id: string): VisualizationSession | undefined {
        return this.sessions.get(id);
    }

    getAllSessions(): VisualizationSession[] {
     ...
     */
    formatForConsole(session: VisualizationSession): string {
        let output = `\n${'═'.repeat(60)}\n`;
