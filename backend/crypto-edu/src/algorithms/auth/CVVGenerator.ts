...
import { encryptDES } from '../../utils/crypto-wrappers';

interface CardData {
    pan: string;
    expiryDate: string; // M MYY or YYMM
    serviceCode: string;
}
...
export class CVVGenerator {
    ...
    staticCVV(cardData: CardData, keyA: Buffer, keyB: Buffer): TraceResult<string> {
        ...
        const block1 = hexToBuffer(hexString.substring(0, 16));
        const block2 = hexToBuffer(hexString.substring(16, 32));

        steps.push({
            name: 'Data Preparation',
            description: 'Construct 2 blocks from PAN+Exp+Svc',
            input: rawData,
            output: `Block1: ${bufferToHex(block1)}\nBlock2: ${bufferToHex(block2)}`
        });

        // Step 2: DES(KA, Block1)
        const step2 = encryptDES(block1, keyA);

        steps.push({
            name: 'Encrypt Block 1',
            description: 'DES(KA, Block1)',
            input: bufferToHex(block1),
            output: bufferToHex(step2)
        });
        ...
        for (let i = 0; i < 8; i++) xorResult[i] = step2[i] ^ block2[i];

        steps.push({
            name: 'XOR Block 2',
            description: 'Result ^ Block2',
            input: bufferToHex(block2),
            output: bufferToHex(xorResult)
        });
        ...
        const tdesResult = encryptDES(
            // Decrypt KB
            crypto.createDecipheriv('des-ecb', keyB, null).setAutoPadding(false).update(
                encryptDES(xorResult, keyA)
            ),
            keyA
        );
        ...
        const encA = encryptDES(xorResult, keyA);
        const decB = crypto.createDecipheriv('des-ecb', keyB, null).setAutoPadding(false).update(encA);
        const encA2 = encryptDES(decB, keyA);

        steps.push({
            name: 'TDES Processing',
            description: 'Enc(KA) -> Dec(KB) -> Enc(KA)',
            input: bufferToHex(xorResult),
            output: bufferToHex(encA2)
        });

        // Step 4: Extract CVV
        // Filter digits from hex result
        const hexResult = bufferToHex(encA2);
        ...
        if (cvv.length < 3) {
            ...
            for (const char of hexResult) {
                ...
                if (val >= 10) extra += (val - 10).toString();
            }
